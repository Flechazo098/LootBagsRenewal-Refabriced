...
class BagStorageMenu(
    ...
    inv: Inventory,
    level: Level,
    private val blockEntity: BagStorageBlockEntity,
    val data: ContainerData
) : LootBagContainerMenu(
    ModMenuTypes.BAG_STORAGE,
    containerId,
    inv,
    level,
    data,
    ...
    constructor(
        ...
        try {
            level.getBlockEntity(extraData.readBlockPos()) as BagStorageBlockEntity
        } catch (e: Exception) {
            BagStorageBlockEntity(BlockPos.ZERO, ModBlocks.BAG_STORAGE.defaultBlockState())
        },
    ...
    override fun stillValid(player: Player): Boolean =
        stillValid(ContainerLevelAccess.create(level, blockEntity.blockPos), player, ModBlocks.BAG_STORAGE)
}

private fun sendSelectLootBagTypePayloadToServer(value: Int) {
    val payload = ServerboundSelectLootBagTypePayload(value)
