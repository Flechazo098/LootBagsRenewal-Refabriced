...
class BagOpenerMenu(
    ...
    constructor(
        ...
        level: Level,
        pos: BlockPos
    ) : this(containerId, inv, level, level.getBlockEntity(pos) as BagOpenerBlockEntity,
        SimpleContainerData(BagOpenerBlockEntity.ContainerDataType.entries.size))
    ...
    private fun addBlockEntityInputSlots() {
        ...
        val top = 19
        for (i in 0 ..< 9) {
            addSlot(LootBagSlot(blockEntity.inputInventory, i, left + i * 18, top, false))
        }
    }
    ...
    private fun addBlockEntityOutputSlots() {
        ...
        for (i in 0 ..< 3) {
            for (j in 0..< 9) {
                addSlot(object : LootBagSlot(blockEntity.outputInventory, j + i * 9, left + j * 18, top + i * 18, true) {
